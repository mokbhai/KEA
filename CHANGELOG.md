...
- The dictation HUD now appears over full-screen apps on macOS instead of only on the desktop Space. `visible_on_all_workspaces` is the only lever Tauri exposes and it sets `NSWindowCollectionBehaviorCanJoinAllSpaces` alone, which covers ordinary Spaces but not a Space another app owns by being full-screen. The overlay's `NSWindow` now also carries `NSWindowCollectionBehaviorFullScreenAuxiliary` and sits at `NSStatusWindowLevel` rather than the floating level `always_on_top` gives it.
